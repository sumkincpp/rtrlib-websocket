...
const int MAX_WEBSOCKET_CLIENTS = 1000;
struct websocket_client clients[1000];
int num_clients = 0;
...
// WebSocket server callback for handling incoming messages
static int websocket_callback(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len)
{
	struct websocket_client *client = NULL;
		...
		case LWS_CALLBACK_CLOSED:
			...
			for (int i = 0; i < num_clients; i++) {
				if (clients[i].wsi == wsi) {
					...
					memmove(&clients[i], &clients[i + 1], (num_clients - i - 1) * sizeof(struct websocket_client));
					num_clients--;
					break;
				}
			}
...
void* websocket_server_run(void *args)
{
	struct WebSocketConfig* config = (struct WebSocketConfig*)args;
...
void send_update_to_clients(const struct pfx_record rec, char* ip, const bool added)
{
	char update[512];	
	snprintf(update, sizeof(update), "{ \"asn\": %d, \"prefix\": \"%s\", \"maxLength\": %u, \"type\": \"%s\" }",
			 rec.asn, ip, rec.max_len, added ? "ADD" : "REMOVE");

	unsigned char* buf = malloc(LWS_PRE + strlen(update));
	memcpy(&buf[LWS_PRE], update, strlen(update));

	for (int i = 0; i < num_clients; i++) {
		struct websocket_client *client = &clients[i];
		lws_write(client->wsi, &buf[LWS_PRE], strlen(update), LWS_WRITE_TEXT);
	}
	free(buf);
}
...
static void update_cb(struct pfx_table *p __attribute__((unused)), const struct pfx_record rec, const bool added)
{
	...
	send_update_to_clients(rec, ip, added);
...
int main(int argc, char **argv)
{
    ...
    pthread_t websocketThread; // Thread ID
	struct WebSocketConfig websocketConfig;
	websocketConfig.port = 8080;

    int err = pthread_create(&websocketThread, NULL, &websocket_server_run, (void*)&websocketConfig);

	if (err != 0) {
		printf("Error creating websocket thread\n");
	}
